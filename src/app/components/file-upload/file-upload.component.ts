...
export class FileUploadComponent {
  ...
  //gets the value from the resolution slider
  getSliderValue(event: any) {
    this.sliderValue = event.value;
  }

  //creates a map object locally and stores it in the database
  uploadFileLocal(ev: Event) {
    ...
    if (
      ...
      if (this.files.length == 1 && this.files[0].type.match('image/*')) {
        this.snackBar.open(
          'At least two images are needed to stitch maps using images. Either upload more than one image, or try uploading a video.',
          '❌'
        );
      ...
      this.api.CreateFlightDetails(flight).catch((e) => {
        console.log(e);
      });
  ...
  //changes the interface so you can select properties of the map
  onFileSelected(event: any) {
    ...
    for (let index = 0; index < event.target.files.length; index++) {
      if (
        !event.target.files[index].type.match('image/*') &&
        !event.target.files[index].type.match('video/*')
      ) {
  ...
  //uploads all selected images to our s3 bucket
  ...
  //splits video into frames and uploads them to s3
  ...
  //splits video into different frames
  ...
  async resizeImage(blobFile: File, width: number, height: number) {
    ...
    return await fromBlob(blobFile, quality, width, height);
  }

  //selects key images and resizes them to be thumbnails
  ...
  //publishes the new stitching job to the web socket
  ...
  //opens the dialog to create a new park
